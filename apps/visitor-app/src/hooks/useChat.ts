...
interface UseChatOptions {
  ...
  onAgentPresence?: (data: { agentId: number; isOnline: boolean }) => void;
}
...
export function useChat(options: UseChatOptions) {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === "message" && data.message) {
          ...
          onError?.(String(data.error ?? "訊息發送失敗"));
          ...
          if (Number.isInteger(agentId) && agentId > 0) {
            onAgentPresence?.({ agentId, isOnline: Boolean(data.isOnline) });
          }
        }
      } catch {
        // ignore parse errors
  ...
  }, [sessionId, visitorNickname, agentToken, onMessage, onError, onSessionUpdate, onUnreadUpdate, onAgentPresence]);
  ...
  const sendFileMessage = useCallback(
    (payload: { fileUrl: string; fileName: string; fileSize: number; mimeType: string }) => {
      if (!sessionId || !wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) return false;
      wsRef.current.send(
        JSON.stringify({
          type: "message",
          sessionId,
